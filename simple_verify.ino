...
void loop() {
  ...
  for (int pulseLength = maxPulseLength; pulseLength > 0; pulseLength -= 100000) {
    ...
    double calculatedCapacitance = calculateCapacitance(pulseLength, pulseCount, resistorValue);

    // Perform the final verification test
    double verifiedCapacitance = finalVerificationTest(pulseLength, pulseCount);
...
double finalVerificationTest(int pulseLength, int pulseCount) {
  pinMode(chargePin, OUTPUT);
  ...
  double actualChargeTime = static_cast<double>(totalTime) / 1000000.0; // Convert from us to s

  // Calculate the actual capacitance using the measured charge time
  double current = supplyVoltage / resistorValue;
  double actualCapacitance = (current * actualChargeTime) / targetVoltage;

  return actualCapacitance * 1000000.0; // Convert F to uF
}
